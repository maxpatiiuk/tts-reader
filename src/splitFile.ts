...
export const retrieveSizeLimit = async (): Promise<number> =>
  new Promise<number>((resolve) =>
    ...
    .catch<number>((error) => {
      console.log(
        `Unable to retrive command length limit. Proceeding with the default of ${formatNumber(
...
function getUniqueFileName(fullName: string, index: number): string {
...
export function splitFile(
  ...
  const getNextFileName = (): string =>
    getUniqueFileName(fileName, chunks.length + 1);
  lines.forEach((line) => {
